    ...
    <div className="flex items-center gap-1.5 px-3 py-2 border-b border-white/5 overflow-x-auto scrollbar-none titlebar-no-drag">
      ...
      {markAllReadEnabled && onMarkAllAsRead && totalUnread > 0 && (
        <button
          ...
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-semibold text-accent-cyan hover:text-white bg-accent-cyan/10 hover:bg-accent-cyan/20 border border-accent-cyan/20 transition-all shrink-0 cursor-pointer"
        >
          <CheckCheck className="w-3.5 h-3.5" />
          <span className="text-[11px] hidden sm:inline">Mark All Read</span>
        </button>
      )}
    </div>
  )
